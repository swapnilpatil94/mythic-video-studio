...
type Manifest = {title: string; duration_seconds: number; platform?: string; asset_kinds?: Record<string, string>; beats: ManifestBeat[]};
...
function FramedLayer({src, zoom, focusY, focusX = 50, camera, depth, progress, direction, reveal, opacity = 1, shiftY = 0, box, fit = 'cover', cameraWeight = 0.35, sway = 0, swayX = 28, swayY = 30, idleScale = 1, seed = 'layer', showPen = false, constructionRegions}: {
  src: string; zoom: number; focusY: number; focusX?: number; camera: MotionFrame; depth: number; progress: number; direction: Direction; reveal?: number; opacity?: number; shiftY?: number; box?: React.CSSProperties; fit?: 'cover' | 'contain'; cameraWeight?: number; sway?: number; swayX?: number; swayY?: number; idleScale?: number; seed?: string; showPen?: boolean; constructionRegions?: ConstructionRegion[];
}) {
  ...
  return <div style={{position: 'absolute', inset: 0, overflow: 'hidden', ...box}}>
    <div style={{position: 'absolute', inset: 0, transform: `rotate(${swayDeg}deg)`, transformOrigin: `${swayX}% ${swayY}%`}}>
      <Img src={src} style={{position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: fit, objectPosition: `${focusX}% ${focusY}%`, transform, transformOrigin: `${focusX}% ${focusY}%`, opacity: opacity * masterOpacity, filter: masterFilter}} />
...
function GeneratedArtwork({beat, progress, beatIndex, format, variant, assetKinds}: {beat: Beat; progress: number; beatIndex: number; format: FormatProfile; variant: number; assetKinds?: Record<string, string>}) {
  ...
  return <div style={{position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden'}}>
    {environment ? (() => {
      ...
      return <FramedLayer key={`${beat.beat_id}-env`} src={staticFile(runtimeAssets[environment])} zoom={isRiver ? 2.6 : Math.max(1.02, shot.zoom * 0.62)} focusY={isRiver ? 92 : 48} camera={camera} depth={0.16} progress={progress} direction={direction} opacity={envOpacity * 0.92} cameraWeight={cameraWeight} idleScale={format.idleAmpScale} seed={`${beat.beat_id}-env`} />;
    })() : null}
