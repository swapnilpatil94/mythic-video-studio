...
function pathFromPoints(points: Point[]) {
  ...
  for (let i = 1; i < usable.length - 1; i += 1) {
    ...
    d += ` Q${current.x.toFixed(2)} ${current.y.toFixed(2)} ${(current.x + next.x) / 2} ${(current.y + next.y) / 2}`;
  }
...
function skeletonize(input: Uint8Array, width: number, height: number) {
  ...
  const neighbors = (x: number, y: number) => [
    image[(y - 1) * width + x], image[(y - 1) * width + x + 1], image[y * width + x + 1], image[(y + 1) * width + x + 1],
    image[(y + 1) * width + x], image[(y + 1) * width + x - 1], image[y * width + x - 1], image[(y - 1) * width + x - 1],
  ];
  ...
  for (let iteration = 0; iteration < 32; iteration += 1) {
    ...
    for (let y = 1; y < height - 1; y += 1) for (let x = 1; x < width - 1; x += 1) {
      const index = y * width + x; if (!image[index]) continue;
      const n = neighbors(x, y); const count = n.reduce((sum, value) => sum + value, 0);
...
function removeTinyComponents(mask: Uint8Array, width: number, height: number, minimum = 6) {
  ...
  for (let y = 1; y < height - 1; y += 1) for (let x = 1; x < width - 1; x += 1) {
    ...
    while (queue.length) {
      const index = queue.pop() as number; component.push(index);
...
function traceSkeletonStrokes(skeleton: Uint8Array, width: number, height: number): SkeletonStroke[] {
  ...
  for (const seed of seeds) {
    ...
    visited.add(pointKey(seed)); const chain = follow(seed); if (chain.length < 4) continue;
    ...
    const xs = simplified.map((point) => point.x); const ys = simplified.map((point) => point.y);
...
function regionAffinity(stroke: SkeletonStroke, regions: ConstructionRegion[], width: number, height: number) {
  ...
  const x = (stroke.centerX / width) * 100; const y = (stroke.centerY / height) * 100; let best = 0;
  for (const region of regions) {
    const radius = Math.max(4, region.radius * 1.9);
    const proximity = clamp01(1 - Math.hypot(x - region.x, y - region.y) / radius);
...
function traceArtwork(image: HTMLImageElement, regions: ConstructionRegion[]): ContourPath[] {
  ...
  const sample = (x: number, y: number) => { const safeX = Math.max(0, Math.min(width - 1, x)); const safeY = Math.max(0, Math.min(height - 1, y)); const i = (safeY * width + safeX) * 4; return {r: pixels[i], g: pixels[i + 1], b: pixels[i + 2], a: pixels[i + 3]}; };

  const luminances: number[] = [];
  for (let y = 2; y < height - 2; y += 3) for (let x = 2; x < width - 2; x += 3) { const p = sample(x, y); if (p.a >= 32) luminances.push(0.2126 * p.r + 0.7152 * p.g + 0.0722 * p.b); }
  luminances.sort((a, b) => a - b);
  const referenceLum = luminances.length ? luminances[Math.floor((luminances.length - 1) * 0.9)] : 220;
  const inkMask = new Uint8Array(width * height);
  for (let y = 1; y < height - 1; y += 1) for (let x = 1; x < width - 1; x += 1) {
    const p = sample(x, y); if (p.a < 32) continue;
    ...
    const darkNeutral = lum < referenceLum - 30 && chroma < 105; const deepNeutral = lum < 112 && chroma < 120; const darkEdge = gradient > 32 && lum < referenceLum - 12 && chroma < 92;
    if (darkNeutral || deepNeutral || darkEdge) inkMask[y * width + x] = 1;
  }

  const cleaned = removeTinyComponents(inkMask, width, height);
  ...
  const boxWidth = Math.max(1, image.clientWidth); const boxHeight = Math.max(1, image.clientHeight);
  ...
  const ordered = [...subject.sort((a, b) => b.affinity - a.affinity || b.stroke.length - a.stroke.length), ...support.sort((a, b) => b.affinity - a.affinity || b.stroke.length - a.stroke.length)];
  const subjectCount = Math.max(1, subject.length); const subjectWeight = subject.reduce((sum, entry) => sum + Math.sqrt(entry.stroke.length), 0); const supportWeight = support.reduce((sum, entry) => sum + Math.sqrt(entry.stroke.length), 0);
  let subjectCursor = 0; let supportCursor = 0;

  return ordered.map((entry, index) => {
    const {stroke} = entry; const weight = Math.sqrt(stroke.length); const isSubject = index < subjectCount;
    let start: number; let end: number;
    if (isSubject) {
      const normalized = weight / Math.max(0.001, subjectWeight); start = subjectCursor * SUBJECT_REVEAL_END; end = Math.min(SUBJECT_REVEAL_END + 0.025, start + Math.max(0.025, normalized * subjectCount * 0.8)); subjectCursor += normalized;
    } else {
      const normalized = weight / Math.max(0.001, supportWeight); start = SUBJECT_REVEAL_END + supportCursor * (FULL_REVEAL_END - SUBJECT_REVEAL_END); end = Math.min(1, start + Math.max(0.018, normalized * support.length * 0.75)); supportCursor += normalized;
    }
    const mapped = stroke.points.map((point) => ({x: (((point.x / width) * naturalWidth * scale + offsetX) / boxWidth) * 100, y: (((point.y / height) * naturalHeight * scale + offsetY) / boxHeight) * 100}));
    const path = pathFromPoints(mapped); const y = stroke.centerY / height;
    return {d: path, start, end, color: INK, width: y > 0.8 ? 0.72 : 0.84, opacity: 0.9};
  }).filter((path) => Boolean(path.d));
}

function Stroke({path, progress}: {path: ContourPath; progress: number}) {
  const local = clamp01((progress - path.start) / Math.max(0.01, path.end - path.start)); if (local <= 0.001) return null;
  return <>
...
export function InkConstructionOverlay({regions, progress, opacity = 1, showGuide = true}: {regions: ConstructionRegion[]; progress: number; opacity?: number; showGuide?: boolean}) {
  const svgRef = useRef<SVGSVGElement | null>(null); const [paths, setPaths] = useState<ContourPath[] | null>(null);
  ...
  useEffect(() => {
    let cancelled = false; let frameHandle: number | null = null; let timeoutHandle: ReturnType<typeof setTimeout> | null = null;
    const startedAt = Date.now(); const parent = svgRef.current?.parentElement;
    ...
    const traceWhenReady = () => {
      ...
      const finish = () => {
        ...
        try {
          ...
          setGeometry({transform: style.transform === 'none' ? 'none' : style.transform, transformOrigin: style.transformOrigin || '50% 50%'});
          continueRender(handle);
        } catch (error) { cancelRender(error); }
      };
      if (image.complete && image.naturalWidth > 0) finish(); else image.addEventListener('load', finish, {once: true});
  ...
  return <svg ref={svgRef} viewBox="0 0 100 100" preserveAspectRatio="none" width="100%" height="100%" style={{position: 'absolute', inset: 0, pointerEvents: 'none', opacity, transform: geometry.transform, transformOrigin: geometry.transformOrigin}} aria-hidden>
    {showGuide ? <g opacity={0.11 * (1 - progressValue)}>
