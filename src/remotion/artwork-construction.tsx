...
function finishingPools(progress: number, seed: string, bounds?: ConstructionRegion): string[] {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    const region: ConstructionRegion = {id: `paper-${i}`, x: clampPct(x), y: clampPct(y), radius: 8 + (i % 4) * 1.8, start: 0.74 + (i % 5) * 0.035, end: 0.90 + (i % 4) * 0.025};
...
export function paintedMaskStyle({progress, seed, regions}: {progress: number; seed: string; regions: ConstructionRegion[]}): React.CSSProperties {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="none"><rect width="100" height="100" fill="black"/>${paths.map((d) => `<path d="${d}" fill="white"/>`).join('')}</svg>`;
...
export function ProgressiveArtwork({src, inkProgress, washProgress, seed, regions, subject, style}: {src: string; inkProgress: number; washProgress: number; seed: string; regions?: ConstructionRegion[]; subject?: ConstructionSubject; style?: React.CSSProperties}) {
  const resolved = regions ?? (subject ? resolveConstructionRegions(subject) : undefined);
  const common: React.CSSProperties = {position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: '50% 50%', ...style};
  if (!resolved || resolved.length === 0) return <Img src={src} style={common}/>;
  return <>
    <Img src={src} style={{...common, ...paintedMaskStyle({progress: inkProgress, seed: `${seed}-ink`, regions: resolved}), filter: 'grayscale(1) sepia(.72) contrast(1.16) saturate(.35)', mixBlendMode: 'multiply'}}/>
    <Img src={src} style={{...common, ...paintedMaskStyle({progress: washProgress, seed: `${seed}-wash`, regions: resolved}), filter: 'saturate(.9) contrast(1.02)'}}/>
  </>;
}
