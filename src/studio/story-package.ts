...
export const WorldBibleSchema = z.object({
  period: z.string().min(1), architecture: z.string().min(1), clothing: z.string().min(1), weapons: z.string().min(1),
  armor: z.string().min(1), jewelry: z.string().min(1), vehicles: z.string().min(1), materials: z.string().min(1),
  environment: z.string().min(1), lighting: z.string().min(1), atmosphere: z.string().optional(),
  forbidden_modern_elements: z.array(z.string()).default([]),
}).strict();

export const StoryPackageVisualBeatSchema = z.object({
  id: z.string().min(1), narration: z.string().default(''), scene_role: z.string().min(1), pace: z.string().default(''),
  shot_type: z.string().default(''), composition: z.string().default(''), visual_action: z.string().default(''),
  animation: z.string().default(''), characters: z.array(z.string()).default([]), props: z.array(z.string()).default([]),
  environments: z.array(z.string()).default([]), camera: z.string().min(1), reveal: z.boolean().default(false),
...
export const StoryPackageSchema = z.object({
  ...
  characters: z.array(CharacterSchema).default([]), environments: z.array(EnvironmentSchema).default([]), props: z.array(PropSchema).default([]),
...
export function splitStoryPackage(raw: unknown, context: {projectId?: string}): SplitResult {
  ...
  for (const vb of pkg.visual_manifest.beats) {
    ...
    beats.push({beat_id: vb.id, duration_seconds: sb.duration_seconds, visual_role: vb.scene_role, asset_refs: assetRefs, camera: vb.camera,
      animation: vb.animation || undefined, text: vb.keyword_text || undefined, narration: sb.narration || vb.narration || undefined,
      sfx: vb.sfx.length ? vb.sfx : undefined, pace: vb.pace || undefined, shot_type: vb.shot_type || undefined,
      composition: vb.composition || undefined, visual_action: vb.visual_action || undefined, reveal: vb.reveal,
      keyword_text: vb.keyword_text || undefined, transition: vb.transition || undefined});
  }
  ...
  const manifest: ProductionManifest = {
    project_id: projectId, title: pkg.story.title, language: pkg.project.language as 'hi-IN', duration_seconds: pkg.project.target_duration_seconds,
    characters: pkg.characters.map((c) => c.id), world: pkg.world as WorldBible | undefined, asset_kinds: assetKinds, asset_sacred: assetSacred,
  ...
  const characters = CharactersSchema.parse({characters: pkg.characters, environments: pkg.environments, props: pkg.props});
...
function splitFlatManifest(raw: unknown, context: {projectId?: string}): SplitResult | undefined {
  ...
  const manifestErrors = validateProductionManifest(raw); if (manifestErrors.length) return {ok: false, errors: manifestErrors.map((e) => `manifest: ${e}`)};
